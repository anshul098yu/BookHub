...
exports.handleBorrowRequest = async (req, res) => {
  try {
    console.log("handleBorrowRequest called with:", req.body);
    ...
    if (!record || record.status !== "pending") {
      console.log("Invalid request - record not found or not pending");
      return res
        .status(404)
...
exports.handleReturnRequest = async (req, res) => {
  try {
    console.log("handleReturnRequest called with:", req.body);
    console.log("User making request:", req.user);
    ...
    if (!record || record.status !== "return_requested") {
      console.log("Invalid request - record not found or not return_requested");
      return res.status(400).json({
        success: false,
        message:
  ...
  try {
    ...
    await Promise.all([
      ...
      mailSender(
        ...
        commonEmailTemplate(
          `Hello ${record.userId.fullName},<br><br>Your book <b>${record.bookId.title
  ...
  try {
    ...
    await Promise.all([
      ...
      mailSender(
        ...
        commonEmailTemplate(
          `Dear ${user.fullName},<br>Your requested book <b>${book.title
...
// Get user's currently issued books
exports.getIssuedBooks = async (req, res) => {
  try {
    const userId = req.user._id;

    // Find all issued books for the user
    const issuedRecords = await BorrowRecord.find({
      userId,
      status: "issued",
    }).populate({
      path: "bookId",
...
exports.getReturnedBooks = async (req, res) => {
  try {
    ...
    // Find all returned books for the user
    const returnedRecords = await BorrowRecord.find({
      userId,
      status: "returned",
    }).populate({
      path: "bookId",
      select: "title authors coverImage availableQuantity rating"
    }).sort({ returnDate: -1 }); // Most recently returned first

    // Extract book data
    const returnedBooks = returnedRecords.map(record => ({
      ...record.bookId.toObject(),
      issueDate: record.issueDate,
    ...
    console.error("Error in getReturnedBooks:", error);
    return res.status(500).json({
      success: false,
      message: "Internal Server Error"
    });
