...
exports.verifyPayment = async (req, res) => {
  ...
  try {
    ...
    if (razorpay_signature === expectedSign) {
      ...
      // Get user's fine amount before updating
      const user = await User.findById(userId);
      const fineAmount = user.fineAmount;

      // Clear user's fine
      await User.findByIdAndUpdate(userId, { fineAmount: 0 });

      // Record the payment in fine history
      if (fineAmount > 0) {
        await FineHistory.create({
          userId: userId,
          amount: fineAmount,
          reason: `Fine paid through Razorpay (Payment ID: ${razorpay_payment_id})`,
          type: "paid",
          paymentId: razorpay_payment_id,
        });
      }
