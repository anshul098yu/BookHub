...
exports.updateBook = async (req, res) => {
  ...
  if (quantity !== undefined) {
    ...
    const newAvailableQuantity = qty - issuedBooks;

    if (newAvailableQuantity < 0) {
      return res.status(400).json({
        success: false,
        message: `Cannot set quantity to ${qty}. Currently ${issuedBooks} books are issued. Minimum quantity should be ${issuedBooks}.`,
      });
    }

    updatedData.quantity = qty;
    updatedData.availableQuantity = newAvailableQuantity;
  }
...
// Fix book availability quantities (calculate from borrow records)
exports.fixBookAvailability = async (req, res) => {
  try {
    const books = await Book.find({ isDeleted: false });
    let fixedCount = 0;
    ...
    for (const book of books) {
      ...
      // Calculate correct available quantity
      const correctAvailableQuantity = book.quantity - issuedBooks;
      ...
      if (book.availableQuantity !== correctAvailableQuantity) {
        ...
        fixedCount++;
        console.log(`Fixed book: ${book.title} - was ${book.availableQuantity}, now ${correctAvailableQuantity}`);
      }
    }

    return res.status(200).json({
      success: true,
      message: `Fixed availability for ${fixedCount} books`,
      fixedCount
    });
  } catch (error) {
    console.error("Error in fixBookAvailability:", error);
    return res.status(500).json({
      success: false,
