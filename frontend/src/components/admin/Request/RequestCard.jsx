...
const RequestCard = ({ requestData, deleteRequestFunction }) => {
  ...
  const handleApproveRequest = async (requestId, actionData) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/borrow/handle-request`,
  ...
  const handleReturnRequest = async (requestId) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/borrow/handle-return`,
        { requestId },
        {
          withCredentials: true,
          headers: {
      ...
      if (response?.data?.success) {
        toast.success(
          response.data.message || "Book return processed successfully"
        );
        deleteRequestFunction(requestId);
      } else {
        toast.error(response?.data?.message || "An error occurred");
      ...
      console.log("Error in handleReturnRequest:", error);
      if (error.response) {
        // Server responded with error status
        toast.error(error.response.data?.message || "Server error occurred");
      } else if (error.request) {
