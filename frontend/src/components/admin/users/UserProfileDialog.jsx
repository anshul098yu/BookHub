...
import FineHistoryCard from "./FineHistoryCard";
...
const UserProfileDialog = ({ userData }) => {
  ...
  const [activeTab, setActiveTab] = useState("borrowing"); // "borrowing" or "fines"
  const [loading, setLoading] = useState({
    fetchBorrowRecordLoading: false,
    fetchFineHistoryLoading: false,
    userStats: false,
  });
  ...
  const [finePagination, setFinePagination] = useState({
    totalRecord: 0,
    currentPage: 1,
    totalPages: 1,
    pageSize: 6,
  ...
  const fetchBorrowRecord = async (page = 1) => {
    ...
    try {
      const response = await axios.get(
        ...
        }/borrow/history?&page=${page}&limit=${pagination.pageSize}&userId=${userData?._id
        }`,
        {
          withCredentials: true,
          headers: {
  ...
  const fetchFineHistory = async (page = 1) => {
    setLoading((prev) => ({ ...prev, fetchFineHistoryLoading: true }));
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BACKEND_URL}/fine/history/${userData?._id}`,
        {
          withCredentials: true,
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
      ...
      if (response?.data?.success) {
        ...
        setFinePagination((prev) => ({
          ...prev,
          totalRecord: response.data.data?.fineHistory?.length || 0,
          currentPage: page,
          totalPages: Math.ceil((response.data.data?.fineHistory?.length || 0) / finePagination.pageSize),
        }));
      }
    } catch (error) {
      toast.error(error?.response?.data?.message || "An error occurred");
    } finally {
      setLoading((prev) => ({ ...prev, fetchFineHistoryLoading: false }));
    }
  };
    ...
    <Dialog>
      ...
      <DialogContent className="!w-[95vw] sm:!w-[90vw] !h-[95vh] !max-w-none sm:!max-w-5xl p-4 sm:p-6 sm:rounded-lg rounded-none overflow-y-auto scrollbar-thin">
        ...
        <div className="space-y-5">
          ...
          <div className="p-4 border rounded-2xl bg-zinc-900">
            ...
            <div className="flex mt-4 flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <div className="flex items-center">
                <Badge
                  className={`font-bold ${userData?.isBlocked ? "bg-red-600" : "bg-green-600"
          ...
          {/* Tabs for Borrowing History and Fine History */}
          <div className="border-b">
            <nav className="flex space-x-6">
              <button
                onClick={() => setActiveTab("borrowing")}
                className={`py-3 px-1 border-b-2 font-medium text-sm ${activeTab === "borrowing"
              ...
              <button
                ...
                className={`py-3 px-1 border-b-2 font-medium text-sm ${activeTab === "fines"
                    ? "border-blue-500 text-blue-600"
                    : "border-transparent text-muted-foreground hover:text-foreground"
                  }`}
              >
          ...
          {activeTab === "borrowing" ? (
            ...
            /* Fine History */
            <>
              {loading.fetchFineHistoryLoading ? (
                <div className="flex justify-center my-10">
                  <Loader width={9} height={40} />
                </div>
              ) : (
                <div className="p-4 border rounded-2xl grid grid-cols-1 gap-4 md:grid-cols-3">
                  {fineHistory.length === 0 ? (
                    ...
                    fineHistory.map((fine) => (
                      <FineHistoryCard key={fine._id} fineData={fine} />
                    ))
                  )}
                </div>
              )}
