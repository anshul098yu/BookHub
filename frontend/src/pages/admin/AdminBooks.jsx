...
import BookImport from "@/components/admin/Books/BookImport";
  ...
  const fetchBooks = async (page = 1) => {
    ...
    try {
      const response = await axios.get(
        ...
        }/book/books?genre=${genre}&language=${language}&availability=${availability}&search=${search}&page=${page}&limit=${pagination.pageSize
        }`,
        {
          withCredentials: true,
          headers: {
  ...
  return (
    <div className="w-full px-4 py-6 text-zinc-900 dark:text-zinc-100 bg-gray-100 dark:bg-[#09090B]">
      ...
      <div className="w-full bg-gray-50 dark:bg-zinc-900 shadow-sm rounded-2xl px-6 py-8 max-w-7xl mx-auto">
        {/* Header*/}
        <div className="flex flex-col gap-4 sm:gap-0 sm:flex-row items-center justify-between mb-6">
          <div>
            <h1 className="text-zinc-900 dark:text-zinc-100 text-3xl font-extrabold">
              Books Management
            </h1>
            <p className="text-customGray text-sm mt-1 font-bold">
              Manage your library's book collection and import from external sources
            </p>
          </div>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-6">
            ...
            <TabsTrigger value="import">Import Books</TabsTrigger>
          </TabsList>
      ...
      {/* Books Section - Only show when on manage tab */}
      {activeTab === "manage" && (
        <>
          {/* Books */}
